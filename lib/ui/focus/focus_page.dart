...
class _TempoFocoPageState extends State<TempoFocoPage> {
  ...
  Widget _buildSettingSlider(BuildContext context,
      {required String label, required double value, required double min, required double max, required ValueChanged<
          double> onChanged,}) {
    final theme = Theme.of(context);
                    ...
                    if (tasks.isEmpty)Padding(
                      ...
                      Flexible(child: ListView.separated(shrinkWrap: true,
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final isSelected = focusProvider.selectedTask?.id ==
                              task.id;
                            ...
                            child: Container(padding: const EdgeInsets.all(16),
                              decoration: BoxDecoration(
                                color: isSelected ? theme.colorScheme.primary
                                    .withValues(alpha: 0.1) : theme.colorScheme
                                    .surface,
                                borderRadius: BorderRadius.circular(16),
                                border: Border.all(color: isSelected
                                    ...
                                    alpha: 0.1),),),
                              child: Row(children: [
                                Icon(isSelected ? Icons.check_circle : Icons
                                    .circle_outlined, color: isSelected
                                    ? theme.colorScheme.primary
                                    : theme.disabledColor,),
                                ...
                                Expanded(child: Column(
                                  ...
                                  children: [
                                    ...
                                    if (task.time.isNotEmpty)Text(task.time,
    ...
    return InkWell(onTap: () => _showTaskSelectionSheet(context, focusProvider),
      ...
      child: Container(padding: const EdgeInsets.all(20),
        decoration: BoxDecoration(color: theme.colorScheme.surface,
          borderRadius: BorderRadius.circular(20),
          border: Border.all(color: hasTask ? theme.colorScheme.primary
              .withValues(alpha: 0.5) : theme.dividerColor.withValues(
              alpha: 0.1), width: hasTask ? 1.5 : 1,
          ),),
        child: Row(children: [
          Container(padding: const EdgeInsets.all(12),
            ...
            child: Icon(focusProvider.status == PomodoroStatus.focus ? Icons
                .menu_book_outlined : Icons.self_improvement,
              color: theme.colorScheme.primary, size: 24,
              ),),
          const SizedBox(width: 16),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(focusProvider.status == PomodoroStatus.focus
                  ? "FOCO ATUAL"
                  : "STATUS", style: TextStyle(color: theme.textTheme.bodyMedium
                  ?.color?.withValues(alpha: 0.6),
                fontSize: 12,
                fontWeight: FontWeight.w600,
                letterSpacing: 1.2,),),
              const SizedBox(height: 4),
              Text(focusProvider.status == PomodoroStatus.focus ? (hasTask
                  ? focusProvider.selectedTask!.title
                  : "Toque para escolher uma tarefa") : "Hora de descansar",
